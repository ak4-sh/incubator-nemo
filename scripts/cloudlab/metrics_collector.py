...
COMBINED_COLUMNS = [
    ...
    "kafkaQueueTimeValidIntervals",
    "kafkaQueueTimeLatestTimestamp",
]
TOPIC_COLUMNS = [
    "timestamp",
    "topic",
...
def read_scaler_metrics(
    work_dir: str,
    am_host: str | None = None,
    log_file: str | None = None,
) -> dict:
    """Read the latest scaler metrics row, falling back to original-Sponge logs."""
    ...
    for line in reversed(_read_tail(am_host, path)):
        ...
        if len(parts) >= 10:
            try:
                ...
                # Newer scaler rows append productive VMWorker usage. Keep old
                # ten-column artifacts readable and distinguish their warm-pool
                # count from an unknown active-worker count.
                metrics["registeredVmWorkers"] = (
                    int(parts[10]) if len(parts) >= 13 else metrics["numLambdaExecutors"]
                )
                metrics["activeVmWorkers"] = int(parts[11]) if len(parts) >= 13 else -1
                metrics["vmWorkerRunningTasks"] = int(parts[12]) if len(parts) >= 13 else -1
                return metrics
            except (ValueError, IndexError):
                continue
    # The original Sponge commit emits this telemetry through ClientRPC rather
    # than a CSV writer. Parsing the already-emitted line keeps the scaler code
    # byte-identical while allowing the experiment collector to remain strict.
    if log_file:
        pattern = re.compile(
            r"Avg cpu:\s+(\S+)\s+Max cpu:\s+(\S+)\s+"
            r"Avg input:\s+(\S+),\s+Avg process input:\s+(\S+)\s+"
            r"Curr input:\s+(-?\d+),\s+NumExecutor:\s+(\d+)"
        )
        for line in reversed(_read_tail(None, log_file, 1000)):
            match = pattern.search(line)
            if match is None:
                continue
            try:
                def finite_or_missing(value: str) -> float:
                    parsed = float(value)
                    return parsed if parsed == parsed and abs(parsed) != float("inf") else -1.0

                return {
                    "avgCpu": finite_or_missing(match.group(1)),
                    "avgInput": finite_or_missing(match.group(3)),
                    "avgProcess": finite_or_missing(match.group(4)),
                    "queue": -1,
                    "numExecutors": int(match.group(6)),
                    "numLambdaExecutors": -1,
                    "registeredVmWorkers": -1,
                    "activeVmWorkers": -1,
                    "vmWorkerRunningTasks": -1,
                }
            except (ValueError, IndexError):
                continue
    return {}
...
def read_source_queue_metrics(
    work_dir: str,
    am_host: str | None = None,
    source_hosts: list[str] | None = None,
    after_timestamp_ms: int | None = None,
) -> dict:
    """Aggregate only newly reported source-task queue-time intervals.

    Each source task reports a one-second mean. To match HoloStream's plot
    semantics, first average those interval means within each task, then take
    an unweighted mean across tasks. ``after_timestamp_ms`` is an exclusive
    watermark that prevents the previous collector poll from being folded
            ...
            if len(parts) >= 10:
                try:
                    ...
                    if after_timestamp_ms is not None and timestamp <= after_timestamp_ms:
                        continue
                    # Some YARN source executors do not receive NEMO_JOB_ID
                    # and therefore report "unknown". The harness deletes
                    # stale source files before each run, while the timestamp
                    # watermark excludes old intervals, so retain these rows.
                    if METRICS_JOB_ID and job_id not in (METRICS_JOB_ID, "unknown"):
                        continue
                    avg_ns = float(parts[5])
                    samples = int(float(parts[7]))
                    if avg_ns >= 0 and samples > 0:
                        seen_intervals.add(interval_key)
                        task_values.setdefault(task_id, []).append(avg_ns)
    ...
    if not task_values:
        return {}
    # HoloStream averages each source's one-second means inside the plot bin,
    # then averages the resulting source-level means without sample weighting.
    values = sorted(sum(intervals) / len(intervals) for intervals in task_values.values())
    ...
    return {
        ...
        "validIntervals": len(seen_intervals),
...
def main():
    ...
    last_valid_source = -1
    source_queue_watermark_ms = int(time.time() * 1000) - INTERVAL_S * 1000
    ...
    metadata = {
        ...
        "queueTimeSamplingEveryRecords": 1000,
        "sourceQueueReportIntervalMs": 1000,
        "collectorIntervalMs": INTERVAL_S * 1000,
        "queueTimeAggregation": (
            "current collector window only; mean one-second intervals per task, "
            "then unweighted mean across tasks"
        ),
        "queueTimeRollingHistory": False,
    }
    try:
    ...
    while True:
        ...
        if source < 0 and committed_offset >= 0:
            # Original Sponge has no source_aggregate_metrics.csv writer. A
            # committed Kafka offset is a conservative source-progress value
            # and is authoritative for terminal drain validation.
        ...
        source_queue = read_source_queue_metrics(
            work_dir,
            am_host,
            source_hosts,
            after_timestamp_ms=source_queue_watermark_ms,
        )
        if source_queue.get("latestTimestamp", -1) > source_queue_watermark_ms:
            source_queue_watermark_ms = source_queue["latestTimestamp"]
        cpu = read_scaler_metrics(work_dir, am_host, log_file)
        ...
        row = {
            ...
            "registeredVmWorkers": cpu.get("registeredVmWorkers", -1),
            "activeVmWorkers": cpu.get("activeVmWorkers", -1),
            "vmWorkerRunningTasks": cpu.get("vmWorkerRunningTasks", -1),
            ...
            "kafkaQueueTimeValidIntervals": source_queue.get("validIntervals", -1),
            "kafkaQueueTimeLatestTimestamp": source_queue.get("latestTimestamp", -1),
        }
